...
export default function AboutClient() {
  ...
  return (
    <div style={{ backgroundColor: 'var(--color-background)', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '64px' }}>
            <div style={{ flex: '1 1 500px' }}>
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
                ...
                <h1 style={{ fontSize: '64px', fontWeight: 700, lineHeight: '1.1', color: 'var(--color-foreground)', marginBottom: '24px', letterSpacing: '-0.02em' }}>
                ...
                <p style={{ fontSize: '20px', lineHeight: '1.7', color: 'var(--color-muted-foreground)', marginBottom: '40px', maxWidth: '600px' }}>
      ...
      <section style={{ padding: '140px 0', backgroundColor: 'var(--color-background)' }}>
      ...
      <section style={{ padding: '140px 0' }}>
        ...
        <div className="container-custom" style={{ maxWidth: '1320px', margin: '0 auto' }}>
          <div style={{ marginBottom: '80px' }}>
            <h2 style={{ fontSize: '48px', fontWeight: 700, color: 'var(--color-foreground)', marginBottom: '24px' }}>Principles that <span style={{ background: 'linear-gradient(135deg, #4f46e5, #0ea5e9)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>guide us</span></h2>
            <p style={{ fontSize: '20px', color: 'var(--color-muted-foreground)', maxWidth: '600px' }}>These core values shape every decision we make, every solution we build, and every relationship we nurture.</p>
          </div>
            ...
            {values.map((val, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <p style={{ fontSize: '18px', lineHeight: '1.7', color: 'var(--color-muted-foreground)' }}>{val.description}</p>
                </motion.div>
              );
            })}
          </div>
      ...
      <section style={{ padding: '140px 0', backgroundColor: 'var(--color-background)', overflow: 'hidden' }}>
        <div className="container-custom" style={{ maxWidth: '1320px', margin: '0 auto' }}>
          <div style={{ marginBottom: '80px' }}>
            <h2 style={{ fontSize: '48px', fontWeight: 700, color: 'var(--color-foreground)', marginBottom: '24px' }}>Milestones that <span style={{ color: '#4f46e5' }}>define us</span></h2>
          </div>
            ...
            {timeline.map((item, idx) => (
              ...
              >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', position: 'relative', zIndex: 1 }}>
                  <div style={{ width: '48px', height: '48px', borderRadius: '24px', backgroundColor: '#4f46e5', color: 'var(--color-background)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '14px', fontWeight: 700, border: '4px solid #f8fafc' }}>
                    <Calendar size={20} />
                  </div>
                  <div style={{ padding: '32px', backgroundColor: 'var(--color-background)', borderRadius: '20px', border: '1px solid var(--color-border)', boxShadow: '0 4px 10px rgba(0,0,0,0.02)' }}>
                    <div style={{ fontSize: '32px', fontWeight: 800, color: '#e2e8f0', marginBottom: '8px' }}>{item.year}</div>
                    <h3 style={{ fontSize: '20px', fontWeight: 700, color: 'var(--color-foreground)', marginBottom: '12px' }}>{item.title}</h3>
                    <p style={{ fontSize: '16px', lineHeight: '1.6', color: 'var(--color-muted-foreground)' }}>{item.description}</p>
                  </div>
                </div>
              </motion.div>
            ))}
      ...
      <section style={{ padding: '140px 0' }}>
        <div className="container-custom" style={{ maxWidth: '1320px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '80px' }}>
            <h2 style={{ fontSize: '48px', fontWeight: 700, color: 'var(--color-foreground)', marginBottom: '24px' }}>Meet Our <span style={{ background: 'linear-gradient(135deg, #4f46e5, #0ea5e9)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Team</span></h2>
            <p style={{ fontSize: '20px', color: 'var(--color-muted-foreground)', maxWidth: '600px', margin: '0 auto' }}>The leadership driving engineering excellence and culture.</p>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '40px' }}>
            {team.map((member, idx) => (
              <motion.div 
                ...
                transition={{ duration: 0.5, delay: idx * 0.1 }}
                whileHover="hover"
                style={{ borderRadius: '20px', border: '1px solid var(--color-border)', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}
              >
                <div style={{ position: 'relative', height: '350px', overflow: 'hidden', backgroundColor: 'var(--color-background)' }}>
                  ...
                  <motion.a href="#" variants={{ hover: { y: 0, opacity: 1 } }} initial={{ y: 20, opacity: 0 }} transition={{ duration: 0.3 }} style={{ position: 'absolute', bottom: '24px', right: '24px', width: '48px', height: '48px', backgroundColor: 'var(--color-background)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#0077b5', textDecoration: 'none', fontWeight: 800, fontSize: '18px' }}>
                    IN
                  </motion.a>
                </div>
                <div style={{ padding: '32px', backgroundColor: 'var(--color-background)', flex: 1 }}>
                  <h3 style={{ fontSize: '24px', fontWeight: 700, color: 'var(--color-foreground)', marginBottom: '4px' }}>{member.name}</h3>
                  <div style={{ fontSize: '16px', color: '#4f46e5', fontWeight: 600, marginBottom: '16px' }}>{member.role}</div>
                  <p style={{ fontSize: '16px', lineHeight: '1.7', color: 'var(--color-muted-foreground)', marginBottom: '24px' }}>{member.bio}</p>
                  <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                    {member.tags.map(tag => (
                      <span key={tag} style={{ padding: '6px 12px', backgroundColor: 'var(--color-background)', borderRadius: '8px', fontSize: '12px', fontWeight: 600, color: 'var(--color-muted-foreground)' }}>
                        {tag}
                      </span>
                    ))}
                  </div>
      ...
      <section style={{ padding: '140px 0', backgroundColor: 'var(--color-background)' }}>
        <div className="container-custom" style={{ maxWidth: '1320px', margin: '0 auto' }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '64px', alignItems: 'center' }}>
            ...
            <div style={{ flex: '1 1 500px' }}>
              <h2 style={{ fontSize: '48px', fontWeight: 700, color: 'var(--color-foreground)', marginBottom: '24px' }}>Your Technology Partner, <span style={{ color: '#4f46e5' }}>Not Just a Vendor</span></h2>
            ...
            <div style={{ flex: '1 1 500px' }}>
              <div style={{ padding: '48px', backgroundColor: 'var(--color-background)', borderRadius: '24px', border: '1px solid var(--color-border)', boxShadow: '0 10px 40px -10px rgba(0,0,0,0.05)', display: 'flex', flexDirection: 'column', gap: '24px' }}>
                {checklist.map((item, idx) => (
                  ...
                  >
                    ...
                    <span style={{ fontSize: '20px', fontWeight: 600, color: 'var(--color-foreground)' }}>{item}</span>
                  </motion.div>
                ))}
              </div>
            </div>
